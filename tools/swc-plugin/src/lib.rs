...
impl VisitMut for TransformVisitor {
  ...
  fn visit_mut_bin_expr(&mut self, e: &mut BinExpr) {
    ...
    if e.op == op!("===") {
      e.left =
        Box::new(Ident::new_no_ctxt(self.config.replace_str.clone().into(), e.left.span()).into());
    }
  }
  ...
  fn visit_mut_ident(&mut self, ident: &mut Ident) {
    ...
    if self
      ...
      .macros
      .iter()
      .any(|m| m.name == ident.sym.to_string())
    {
...
pub fn process_transform(program: Program, metadata: TransformPluginProgramMetadata) -> Program {
  ...
  let config = serde_json::from_str::<Config>(
    &metadata
      ...
      .expect("failed to get plugin config for TransformVisitor"),
  )
...
fn test_plugin_transform() {
  ...
  test_transform(
    ...
    "foo === bar; i18n.t(\"hello\");",
    "import { i18n as i18n } from \"macros\"; leo === bar; i18n.t(\"hello\");",
  );
}
...
//   boo,
//   // Input codes
//   r#"foo === bar; i18n.t("hello");"#,
//   // Output codes after transformed with plugin
//   // test will not pass since not specify "isModule" param as true
//   r#"import { i18n as i18n } from "macros"; leo === bar; i18n.t("hello");"#
// );
